        ...
        if(isset($_POST['action_name']) && isset($_POST['current_user_id'])) {
            ...
            if(mysqli_num_rows($result_s) == 1) {
                ...
                if($valid_action_name == 1) {
                    ...
                    $sql_get_total_no_of_friend_in_login_user = "SELECT total_friends FROM user WHERE user_id=$login_user_id";
                    $result_get_total_no_of_friend_in_login_user = mysqli_query($conn,$sql_get_total_no_of_friend_in_login_user) or die("Query Failed get_total_no_of_friend_in_login_user");
                    $arr_get_total_no_of_friend_in_login_user = mysqli_fetch_assoc($result_get_total_no_of_friend_in_login_user);
                    ...
                    $total_friends = 0;
                    if($arr_get_total_no_of_friend_in_login_user['total_friends'] < 1000) {
                        $total_friends = $arr_get_total_no_of_friend_in_login_user['total_friends'];
                    }
                    else if($arr_get_total_no_of_friend_in_login_user['total_friends'] < 1000000){
                        $total_friends = (cal_num_($arr_get_total_no_of_friend_in_login_user['total_friends'] / 1000))."K";
                    }
                    else {
                        $total_friends = (cal_num_($arr_get_total_no_of_friend_in_login_user['total_friends'] / 1000000))."M";
                    }
                    $info['total_no_of_friend'] = $total_friends;

                    if($arr_get_total_no_of_friend_in_login_user['total_friends'] > 0) {
                        $info['total_no_of_friend_status'] = true;
                    }
                    else {
                        $info['total_no_of_friend_status'] = "<div class='col-12 my-3 d-flex justify-content-center align-items-center' style='background-color: #fff; box-shadow: 0px 0px 4px 2px #000, 0px 0px 4px 2px #fff, 0px 0px 4px 2px #000; padding: 10px 0px; border-radius: 6px; font-weight: bold; font-size: 20px;user-select: none;'>No Friend Found</div>";
                    }
