<?php 
    if($_SERVER['REQUEST_METHOD'] == "POST") {
        $info = array();
        if (isset($_POST["post_id"]) && isset($_POST["img_name"])) {
            $info["error"] = 1;

            require_once "../conn.php";
            $conn = connected() or die("Connection Failed");
            $post_id = mysqli_real_escape_string($conn,$_POST['post_id']);
            $img_name = mysqli_real_escape_string($conn,$_POST['img_name']);
            ...
            if(mysqli_num_rows($result) > 0) {
                ...
                if($post_status == 1) {
                    ...
                    if($all_photo_video_in_s_p != "") {
                        ...
                        if(in_array($img_name,$user_all_photo_video_arr)) {
                            $new_img_video_ = "";
                            $i=0;
                            foreach($user_all_photo_video_arr as $img_video) {
                                if($img_name != $img_video) {
                            ...
                            unlink("../post_img/".$img_name);
                            $info['success'] = 1;

                            $sql_c_image = "SELECT post_id,post_img_name FROM post WHERE user_id = $user_id";
                            $result_c_img = mysqli_query($conn,$sql_c_image) or die("Query Failed c img");

                            $image_arr = array();
                            $i=0;
                            while($arr_get_image_video = mysqli_fetch_assoc($result_c_img)) {
                            ...
                            if(count($image_arr) > 0) {
                                ...
                                $info['img_count'] = "<div class='col-12 d-flex justify-content-center align-items-center' style='padding-top: 5px;font-weight: bold; font-size: 20px;user-select: none;'>No Photo Found</div>";
                            }
                        }
                        else {
                            $info["error"] = 0;
